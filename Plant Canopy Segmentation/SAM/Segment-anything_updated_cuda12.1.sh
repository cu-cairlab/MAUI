...
while [[ "$#" -gt 0 ]]; do
    case $1 in
		...
		-v_size_ran|--voxel_size_ran) VOXEL_RAN="$2" ; shift ;; #voxel_size for RANSAC
		-v_size_icp|--voxel_size_icp) VOXEL_ICP="$2" ; shift ;; #voxel_size for ICP
		-row_col|--field_num_row) FIELD_NUM_ROW="$2" ; shift ;; #number of rows/number of plants per col
		-tilt|--tilt_angle) TILT="$2" ; shift ;; #tilt angle to help straighten image for grid
		-meta|--metadata_file) META="$2" ; shift ;; #metadata file name
...
case ${PROG_VALUE} in 
    ...
    docker run \
            ...
            segment-anyhemp:4.0_cuda12.1_published\
    ...
    docker run \
            ...
            --volume /tmp/.X11-unix:/tmp/.X11-unix\
            segment-anyhemp:4.0_cuda12.1_published\
			/home/miniconda3/bin/python -u /home/ubuntu/Hemp_Characterization-master/dbscan_seg_plant.py  --data_dir  /home/ubuntu/input --data_date ${MAP_NAME} \
            --width ${WIDTH} --height ${HEIGHT} --exg_th ${EXG_TH} --kernel_size ${KERNEL_SIZE} --minimum_counts ${MIN_COUNT} --field_num_col ${FIELD_NUM_COL} --first_plant_id ${F_ID} --plants_per_col ${FIELD_NUM_ROW} 
    ;;
	...
	do
		 ...
		 docker run \
            ...
            --volume /tmp/.X11-unix:/tmp/.X11-unix\
            segment-anyhemp:4.0_cuda12.1_published\
			/home/miniconda3/bin/arosics global /home/ubuntu/input/RGBmaps/${MAP_NAME}_RGB.tif /home/ubuntu/input/RGBmaps/${curr}_RGB.tif  -o /home/ubuntu/input/RGBmaps/Coregistered/"${curr}"_to_${MAP_NAME}_RGB.tif -fmt_out GTIFF -max_shift 200 
