...
if len(list(args.id_vars))>1:
	groupby_plantid = outgrid_df.groupby(list(id_vars)[:-1])
	as_df = groupby_plantid.mean()
	as_df.sort_values(by=id_vars, inplace=True)
...
elif len(list(args.id_vars))==1:
	groupby_plantid = outgrid_df.groupby(list(id_vars))
	as_df = groupby_plantid.mean()
	as_df.sort_values(by=id_vars, inplace=True)
	as_df.reset_index(inplace=True)
	final_df = as_df.drop(['index'], axis=1)
